...
int p2p_test(pci_bar_handle_t pci_bar_handle, uint64_t address, uint32_t length, int num_trials, int test);
...
int p2p_example(int num_slot, int GROUP) {
    ...
    //         sleep(1);
    //
	      		 //rc = p2p_test(pci_bar_handle[slot_id], address[tgt_slot_id], 0, 31, 0); // Round trip (length = 0, trials = 31, test = 0)
	      		 rc = p2p_test(pci_bar_handle[slot_id], address[tgt_slot_id], 0xFF, 31, 1); // Throughput (length = 0x7F, trials = 31, test = 0)
...
int p2p_write(pci_bar_handle_t pci_bar_handle, uint64_t address, uint32_t test_pattern) 
{
    ...
    rc = fpga_pci_poke(pci_bar_handle, 0x0, CONT_MODE| INCR_DATA); // Write out data and increment; Do not compare on read

    read_data = (((MAX_INST -1)<<16) + ((MAX_INST-1)));	// Read and write max number of instructions
...
/*
 * Function which issues read to measure round trip latency 
 *
 * pci_bar_handle - struct for receiving FPGA,
 * address - address for receiving FPGA
 * length - number of data words to read
 * num_trials - number of RTT measurements
 * test - 0 or 1 (0 round trip, 1 throughput)
 *
 * returns 1 on failue
 */
int p2p_test(pci_bar_handle_t pci_bar_handle, uint64_t address, uint32_t length, int num_trials, int test)
{
    int no_of_outstd = 0x1f;
    //int length = 0x0;
    uint32_t test_pattern = 0x0BADF00D;     // Bogus test pattern not used in this test
    int rc;
    uint32_t addr_low, addr_high;
    uint32_t read_data; 
    //uint32_t bresp_data, rresp_data; 
    ...
    uint64_t base_addr_scale;
    uint64_t value;
    uint32_t config_reg, cfg_write; 

    // Set test type bit for address 0xE00
    if (test) {
        cfg_write = 0x2;
        log_info("INFO: P2P UCB Test: Throughput");
    } else {
        cfg_write = 0x00;
        log_info("INFO: P2P UCB Test: Round Trip");
    }
    ...
    /* write a read outstanding on 1 */
    rc = fpga_pci_poke(pci_bar_handle, ATG_RD_OUTSTD, no_of_outstd);    // Write number of max outstanding read requests allowed

    /* configure cl_tst */
    log_info ("INFO: Configuring the cl_tst module ");
    rc = fpga_pci_poke(pci_bar_handle, 0x0, 0x01000008);        // Non sync and constant data word

    read_data = (((MAX_INST -1)<<16) + ((MAX_INST-1)));
    rc = fpga_pci_poke(pci_bar_handle, ATG_INST_CNT, read_data);

    // Configure test unit
    log_info ("INFO: Configuring the p2p_test module ");
    fpga_pci_poke(pci_bar_handle, 0xE08, 0x7735940);   		// Write pause time
    fpga_pci_poke(pci_bar_handle, 0xE04, num_trials);   						// Write number of trials
    fpga_pci_poke(pci_bar_handle, 0xE00, cfg_write | 0x1);  // Write Start 
    fpga_pci_poke(pci_bar_handle, 0xE00, cfg_write & 0xFFFFFFFE);  // Write Start to stop

    sleep(1);
    // Spin and wait for busy to be deasserted 
    log_info ("INFO: Spinning on busy ");
    fpga_pci_peek(pci_bar_handle, 0xE00, &config_reg);
    while(config_reg == 0x1) {
        fpga_pci_peek(pci_bar_handle, 0xE00, &config_reg);
    }
    ...
    for (int x = 0; x < num_trials; x++) {
        tmp = TRIAL_BASE + 8*x;
				fpga_pci_peek(pci_bar_handle, tmp + 4, &config_reg);
        value = config_reg;
        fpga_pci_peek(pci_bar_handle, tmp, &config_reg);
        value = (value << 32) + config_reg;
				
        // Log info
    //    log_info("\tINFO: READING ADDRESSES  %x - %x: ", tmp, tmp+4);
        log_info("\tRESULTS: Trial %x: %lu (%f sec) ", x, value, value * 1.0/125.0e6);
    }
